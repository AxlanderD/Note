...
-----
    限定符号：
     + 号代表前面的子表达式至少出现一次（可以出现1次或多次） 如lee+t 可以匹配 leeeet leeet等
     * 号代表前面的子表达式可以出现任意次数（包括0次）
     ? 号代表前面的子表达式最多出现一次（可以出现0次或1次） 如 colou?r 可以匹配color colour
     {n} 确定匹配前面的子表达式n次
     {n,} 匹配前面的子表达式至少n次
...
![图片链接的文字](http://pic37.nipic.com/20140113/8800276_184927469000_2.png)
```
代码块的写法：
    1.前面空4格
    2.使用``` content ```
